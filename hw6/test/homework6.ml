...
(* Helper function: recursively checks that all subtrees are AVL *)
 let rec makeAVL aBST =
   match aBST with
     | Empty -> Empty        
     | Node(node,left,right) ->

                     (* Makes the subtrees AVL *)
                     let leftAVL = (makeAVL left) in
                     let rightAVL = (makeAVL right) in 
 
		     let anAVL = Node(node,leftAVL,rightAVL) in   
 
                     (* If left heavy *)
                     if (((height leftAVL) - (height rightAVL)) > 1)
                      
                       then (rotate_right anAVL) 

                     (* If right heavy *)
                     else if (((height rightAVL) - (height leftAVL)) > 1)
...
(* Remnants of an older attempt - gave values in wrong positions
let rec makeAVL aBST =
   match aBST with
     | Empty -> Empty        
     | Node(node,left,right) ->
                     ...
                     if (((height left) - (height right)) > 1)
                       then 
                          (* Ensure In Lowest Case *)
                          if (((height left) - (height right)) = 2)
                              then 
                               match left with
                                | Empty -> Empty        
                                | Node(nodeL,leftL,rightL) ->

                                (* Left-Right Case *)
                                if ((height rightL) > (height leftL))
                                  then (rotate_right (Node(node,(rotate_left left),right)))
                                else (* Left-Left Case *)
                                  (rotate_right aBST)
                       ...
                       then 
                          ...
                          if (((height right) - (height left)) = 2)
                              then 
                               match right with
                                | Empty -> Empty        
                                | Node(nodeR,leftR,rightR) ->

                                (* Right-Left Case *)
                                if ((height leftR) > (height rightR))
                                  then (rotate_left (Node(node,left,(rotate_right right))))
                     ...
                     (* If already AVL *)
                       ...
                       Node(node,(makeAVL left),(makeAVL right))
*)
...
let rec eval (exp:exp) (env:int env):int = 
  match exp with
    ...
    | Not (arg) -> if ((eval arg env) != 0) then 0 else 1
    | If (val1, val2, val3) -> if ((eval val1 env) != 0)
                                 then (eval val2 env)
                               else
                                 (eval val3 env)
    | Letval (x, val1, val2) -> (eval val2 (update env x (eval val1 env)))
    | Letfun (_,_,_,_)  -> failwith "Letfun not implemented" 
    | Letrecfun (_,_,_,_) -> failwith "Letrecfun not implemented"
    | App (_,_)  -> failwith "App not implmented"
